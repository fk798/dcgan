...
parser.add_argument("--num_pages", type=int, help="number of pages to scrape (leave blank to download all of them)", default=10)
...
def get_painting_list(count, typep, searchword):
    print(searchword)
    try:
        time.sleep(5.0)  # random sleep to decrease concurrence of requests
...
def downloader(link, genre, output_dir):
    ...
    try:
        time.sleep(1)  # try not to get a 403
